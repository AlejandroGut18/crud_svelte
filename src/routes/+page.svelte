...
// @ts-nocheck
	...
	let cancionEditando = $state (null);
	...
	const agregarCancion = (cancion) => {
		const nuevoId = canciones.length > 0 ? Math.max(...canciones.map((c) => c.id)) + 1 : 1;
		canciones = [...canciones, { ...cancion, id: nuevoId }];
		mostrarModal = false;
	};
			...
			{#each cancionesFiltradas() as cancion}
				<tr>
					...
					<td>
						...
						<BotonesAcciones
							editarCancion={() => abrirModal(cancion)}
							eliminarCancion={() => eliminarCancion(cancion.id)}
						/>
					</td>
				</tr>
			{/each}
